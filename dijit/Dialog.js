define(
	...
	function(require,
			...
			],
			{
				...
				_position : function() {
					...
					if (!domClass.contains(this.ownerDocumentBody, "dojoMove")) { // don't do anything if called during auto-scroll
						var node = this.domNode, viewport = winUtils.getBox(this.ownerDocument), p = this._relativePosition, bb = p ? null
							: domGeometry.position(node), l = Math.floor(viewport.l + (p ? p.x : (viewport.w - bb.w) / 2)), t = Math
							.floor(viewport.t + (p ? p.y : (viewport.h - bb.h) / 2));
						domStyle.set(node, {
							left : l + "px",
							top : t + "px"
						});
				...
				_onKey : function _onKey(/* Event */evt) {
					// summary:
		...
		var Dialog = declare("dijit.Dialog",
		[
			ContentPane,
			_DialogBase
		], {
		...
		var DialogLevelManager = Dialog._DialogLevelManager = {
			...
			show : function(/* dijit/_WidgetBase */dialog, /* Object */underlayAttrs) {
				...
				var zIndex = ds[ds.length - 1].dialog ? ds[ds.length - 1].zIndex + 2
			...
			hide : function hide(/* dijit/_WidgetBase */dialog) {
				// summary:
